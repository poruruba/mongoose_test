...
//    const i2c = require('i2c-bus');
    ...
    this.i2cAddress = i2cAddress ? i2cAddress : BME280.BME280_DEFAULT_I2C_ADDRESS();
...
//            console.log(`Found BMx280 chip ID 0x${chipId.toString(16)} on bus i2c-${this.i2cBusNo}, address 0x${this.i2cAddress.toString(16)}`);
            console.log(`Found BMx280 chip ID 0x${chipId.toString(16)}, address 0x${this.i2cAddress.toString(16)}`);
  ...
  readSensorData() {
    return new Promise((resolve, reject) => {
      ...
      this.i2cBus.readI2cBlock(this.i2cAddress, this.REGISTER_PRESSURE_DATA, 8, buffer, (err, bytesRead, buffer) => {
        if(err) {
          return reject(err);
        }
  ...
  async loadCalibration(callback) {
    ...
    this.i2cBus.readI2cBlock(this.i2cAddress, this.REGISTER_DIG_T1, 24, buffer, async (err, bytesRead, buffer) => {
      let h1   = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H1);
      let h2   = await this.i2cBus.readWordSync(this.i2cAddress, this.REGISTER_DIG_H2);
      let h3   = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H3);
      let h4   = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H4);
      let h5   = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H5);
      let h5_1 = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H5 + 1);
      let h6   = await this.i2cBus.readByteSync(this.i2cAddress, this.REGISTER_DIG_H6);
